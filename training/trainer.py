...
def clear_memory():
    """Clear memory - simplified for CPU"""
    gc.collect()
...
def calcc(input_batch, target_batch, model, device):
    ...
    for i in range(len(input_batch)):
        inp = input_batch[i].to(device)   
        logits = model(inp)
        del inp
        tgt = target_batch[i].to(device)
...
def trainer(model, train_loader, val_loader, device='cpu'):
    learning_rate = 3e-4        
    max_iters = 5         
    warmup_steps = 100    
    min_lr = 3e-5           
    ...
    if os.path.exists('model/gptoss.pt'):
        model.load_state_dict(torch.load('model/gptoss.pt', map_location=device))
